<template>
 ...
 ERROR: {{ formFeedback.courseCode }}</span></div>
...
const isPasswordValid = computed(() => {
  ...
  if (studentForm.password.length < 8) {
    formFeedback.password = 'password must be at least 8 characters'
    return false
  }
  return true
})
const isNameValid = computed(() => {
  ...
  if (studentForm.name.length > 15) {
    formFeedback.name = 'name must be at most 50 characters'
    return false
  }
