    ...
    <div v-if="student">
      ...
      <form @submit.prevent="send">
        <div>Subject:
          ...
          <span
              v-if="formData.subject !== null && !isSubjectValid"
              class="error">
 ERROR: {{ formFeedback.subject }}</span></div>
        <div>Body:
          <input v-model="formData.body">
          <span v-if="formData.body !== null && !isBodyValid" class="error">
 ERROR: {{ formFeedback.body }}</span></div>
...
const isBodyValid = computed(() => {
  if ( ! formData.body ) {
    formFeedback.body = 'message body is required'
    return false
  }
  return true
})
